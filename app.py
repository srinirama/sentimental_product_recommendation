...
data = pd.read_csv('./dataset/users.csv', header=0)
...
def home():
    ...
    myData = [item for sublist in data.values.tolist() for item in sublist]
    #print(myData)

    return render_template('index.html', _data=myData)
...
def prediction():
    ...
    if(not(items is None)):
        ...
        return render_template("index.html", column_names=items.columns.values, row_data=list(items.values.tolist()), zip=zip, _user=user)
    else:
